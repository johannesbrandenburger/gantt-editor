...
<template>
    ...
    <div style="height: 100vh; width: 100%; margin: 0 auto;">
        <GanttEditor
            ...
            @onChangeDestinationId="(slotIds, newDestinationId) => console.log(`onChangeDestinationId(${slotIds.join(',')}, ${newDestinationId})`)"
            @onCopyToDestinationId="(slotIds, newDestinationId) => console.log(`onCopyToDestinationId(${slotIds.join(',')}, ${newDestinationId})`)"
            @onMoveSlotOnTimeAxis="(slotIds, timeDiff) => console.log(`onMoveSlotOnTimeAxis(${slotIds.join(',')}, ${timeDiff})`)"
            @onCopySlotOnTimeAxis="(slotIds, timeDiff) => console.log(`onCopySlotOnTimeAxis(${slotIds.join(',')}, ${timeDiff})`)"
